...
export function NewHabitForm() {
  ...
  function handleToggleWeekDay(weekDay: number) {
    if (weekDays.includes(weekDay)) {
      ...
      console.log(weekDaysWithRemovedOne)
  ...
  return (
    <form onSubmit={createNewHabit} className="w-full flex flex-col mt-6">
      ...
      <input
        type="text"
        id="title"
        placeholder="exe.: Exercícios, dormir bem, etc"
      ...
      <button
        type="submit"
        className="mt-6 rounded-lg p-4 gap-3 flex item-center justify-center font-semibold bg-green-600 hover:bg-green-500 transition-colors focus:outline-none focus:ring-2 focus:ring-green-600 focus:ring-offset-2 focus:ring-offset-zinc-900"
      >
